...
void CStage1::LoadData(void)
{
	HANDLE	hFile = CreateFile(L"../Data/Stage1.dat",
		GENERIC_READ,
		0,
		nullptr,
		OPEN_EXISTING,
...
void CStage1::RenderBack(void)
{
	...
	for (int i = 0; i < 120; ++i)
	{
		...
		D3DXMatrixTranslation(&matTrans, i*(MAPX/120) + 20 + g_vScroll.x + MAPX, MAPY/2 + g_vScroll.y, 0.f);
		...
		if (i < 20)
		{
			m_pDevice->GetSprite()->Draw(
				...
				NULL,
				D3DCOLOR_ARGB(255, 250 - (20 - i) * 5, 140 - (20 - i)*5, 140 - (20 - i)*5));
		}
		else if (i >= 100)
		{
			m_pDevice->GetSprite()->Draw(
				...
				&D3DXVECTOR3(20, MAPY / 2.f, 0.f),
				NULL,
				D3DCOLOR_ARGB(255, 250 - (i - 99) * 5, 140 - (i - 99)*5, 140 - (i - 99)*5));
		}
		else
		{
			m_pDevice->GetSprite()->Draw(
...
void CStage1::CollisionCheck(void)
{
...
void CStage1::Render(void)
{
	for (auto& map2 : m_Objlist[OBJ_MAPRECT])
		static_cast<CMaprect*>(map2)->Render(1);
