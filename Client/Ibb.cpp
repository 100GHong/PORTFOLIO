...
void CIbb::CheckDirection(void)
{
	...
	DWORD	dwKey = CNetworkMgr::GetInstance()->GetCData().m_iKey;
	

	if (dwKey & KEY_LEFT)//GetAsyncKeyState(VK_LEFT))
	{
		m_vDir.x = -m_fXSpeed;
	...
	else if (dwKey & KEY_RIGHT)//GetAsyncKeyState(VK_RIGHT))
	{
		m_vDir.x = m_fXSpeed;
	...
	if (dwKey & KEY_UP)//GetAsyncKeyState(VK_UP))
	{
		if ((m_fJumpTime <= 0.f))
		{
			if (m_vDir.y == 0.f)
